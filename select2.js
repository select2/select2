  ...
  function local(options) {
    ...
    return function (query) {
      ...
      process = function(datum, collection) {
        var group, attr;
        datum = datum[0];
        if (datum[options.childrenProperty]) {
          group = {};
          for (attr in datum) {
            if (datum.hasOwnProperty(attr)) group[attr]=datum[attr];
          }
          group[options.childrenProperty]=[];
          $(datum[options.childrenProperty]).each2(function(i, childDatum) { process(childDatum, group[options.childrenProperty]); });
          if (group[options.childrenProperty].length || query.matcher(t, text(group), datum)) {
      ...
      if (select) {
        ...
        if (!("query" in opts)) {
          ...
          if ("ajax" in opts) {
            ...
            opts.data.childrenProperty = opts.childrenProperty;
