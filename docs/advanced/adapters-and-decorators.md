...
You can find more information on how to integrate Select2 with your existing AMD-based project [here](../getting-started/builds-and-modules.md). Select2 automatically loads some modules when the adapters are being automatically constructed, so those who are using Select2 with a custom AMD build using their own system may need to specify the paths that are generated to the Select2 modules.
