...
In this case, Option 2 will be pre-selected, and Option 3 will be [disabled](../options.md#disabling-options).
...
> [!WARNING]
> Because Select2 generates an `<optgroup>` when creating nested options, only [a single level of nesting is supported](../options.md#dropdown-option-groups). Any additional levels of nesting is not guaranteed to be displayed properly across all browsers and devices.
